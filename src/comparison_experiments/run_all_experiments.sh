#!/usr/bin/env bash
# 一次性运行 DoS、CTU-13、Scan、XSS 和 Zeus 五组实验。
...
usage() {
    cat <<'EOF'
用法：
  comparison_experiments/run_all_experiments.sh [选项]

该脚本按顺序运行：DoS、CTU-13 场景 12、Scan、XSS、Zeus。
...
echo "========== 开始 dos 实验 =========="
DOS_ARGS=(--ids-csv "${IDS_CSV}")
if [[ "${#COMMON_ARGS[@]}" -gt 0 ]]; then DOS_ARGS+=("${COMMON_ARGS[@]}"); fi
if [[ "${KEEP_WORK}" -eq 1 ]]; then DOS_ARGS+=(--keep-work); fi
"${SCRIPT_DIR}/dos/run_full_experiment.sh" "${DOS_ARGS[@]}"
echo "========== 完成 dos 实验 =========="

echo
echo "========== 开始 ctu13_scenario12 实验 =========="
CTU_ARGS=(--ctu-pcap "${CTU_PCAP}" --ids-csv "${IDS_CSV}")
if [[ "${#COMMON_ARGS[@]}" -gt 0 ]]; then CTU_ARGS+=("${COMMON_ARGS[@]}"); fi
if [[ "${KEEP_WORK}" -eq 1 ]]; then CTU_ARGS+=(--keep-work); fi
"${SCRIPT_DIR}/ctu13_scenario12/run_full_experiment.sh" "${CTU_ARGS[@]}"
echo "========== 完成 ctu13_scenario12 实验 =========="

run_attack scan --positive-pcap "${SCAN_PCAP}"
run_attack xss --positive-pcap "${XSS_PCAP}"
run_attack zeus --positive-pcap "${ZEUS_PCAP}"

echo
echo "五组实验全部完成。结果目录：${PROJECT_ROOT}/comparison_results"
